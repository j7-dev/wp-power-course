...
class BundleProductQuantitiesTest extends TestCase {
	 ...
	 * @group smoke
	 *
	 * 確認 Helper 類別有 get_product_quantity 方法
	 */
	public function test_Helper_有get_product_quantity方法(): void {
		$helper = Helper::instance( $this->bundle_id );
		$this->assertNotNull( $helper );
		$this->assertTrue(
			method_exists( $helper, 'get_product_quantity' ),
			'Helper 應有 get_product_quantity() 方法'
		);
	}
	 ...
	 * Example: 未指定數量時預設為 1
	 */
	public function test_get_product_quantity_無資料時回傳預設1(): void {
		// Given 有一個銷售方案（無 pbp_product_quantities meta）
		$helper = Helper::instance( $this->bundle_id );
		$this->assertNotNull( $helper, '應能建立 Helper 實例' );

		// When 查詢某商品的數量（無資料）
		$qty = $helper->get_product_quantity( $this->tshirt_id );
	 ...
	 *
	 * 注意：此測試驗證 Helper::get_product_quantity() 計算邏輯
	...
	public function test_訂單庫存扣減_購買1份按商品數量扣(): void {
		...
		$course_qty_per_product = $helper->get_product_quantity( $this->course_id );
		$tshirt_qty_per_product = $helper->get_product_quantity( $this->tshirt_id );
	 ...
	 *
	 * Feature: 管理銷售方案
	 * 向後相容：不在 quantities 中的商品，get_product_quantity 應回傳 1
	 */
