...
### 步驟一 — 啟用 MCP 伺服器（WordPress 後台）

1. 登入 WordPress 後台
2. 前往 **Power Course → 設定 → MCP**
3. 將 MCP 伺服器切換為**啟用**
4. （選填）選擇要啟用的工具分類 — 預設全部 9 個分類（41 個工具）皆啟用
...
> **注意**：`settings.json` 是管理權限與偏好的設定檔，MCP 伺服器**不能**寫在裡面。
