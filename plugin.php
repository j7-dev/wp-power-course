...
if ( ! \class_exists( 'J7\PowerCourse\Plugin' ) ) {
	...
	final class Plugin extends Singleton {
		...
		const APP_NAME    = 'Power Course';
		const KEBAB       = 'power-course';
		const SNAKE       = 'power_course';
		const GITHUB_REPO = 'https://github.com/j7-dev/wp-power-course';
		...
		public function register_required_plugins(): void {
			...
			$config = array(
				...
				'dismiss_msg'  => __( '這個訊息將在依賴套件被安裝並啟用後消失。' . Plugin::APP_NAME . ' 沒有這些依賴套件的情況下將無法運作！', 'power_course' ), // If 'dismissable' is false, this message will be output at top of nag.
				'is_automatic' => true, // Automatically activate plugins after installation or not.
				'message'      => '', // Message to output right before the plugins table.
				'strings'      => array(
					'page_title'                      => __( '安裝依賴套件', 'power_course' ),
					'menu_title'                      => __( '安裝依賴套件', 'power_course' ),
					'installing'                      => __( '安裝套件: %s', 'power_course' ), // translators: %s: plugin name.
					'updating'                        => __( '更新套件: %s', 'power_course' ), // translators: %s: plugin name.
					'oops'                            => __( 'OOPS! plugin API 出錯了', 'power_course' ),
					...
					'return'                          => __( '回到安裝依賴套件', 'power_course' ),
					'plugin_activated'                => __( '套件啟用成功', 'power_course' ),
					'activated_successfully'          => __( '以下套件已成功啟用:', 'power_course' ),
					// translators: 1: plugin name.
					'plugin_already_active'           => __( '沒有執行任何動作 %1$s 已啟用', 'power_course' ),
					// translators: 1: plugin name.
					'plugin_needs_higher_version'     => __( Plugin::APP_NAME . ' 未啟用。' . Plugin::APP_NAME . ' 需要新版本的 %s 。請更新套件。', 'power_course' ),
					// translators: 1: dashboard link.
					'complete'                        => __( '所有套件已成功安裝跟啟用 %1$s', 'power_course' ),
					'dismiss'                         => __( '關閉通知', 'power_course' ),
					'notice_cannot_install_activate'  => __( '有一個或以上的依賴/推薦套件需要安裝/更新/啟用', 'power_course' ),
					'contact_admin'                   => __( '請聯繫網站管理員', 'power_course' ),
